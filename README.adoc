...
The FIDO Metadata Service (MDS) snapshot is now managed entirely by a scheduled Render cron job instead of the Flask runtime.

- Runtime downloads and signature verification are disabled; the web server only reads the previously verified JSON payload.
- Render runs `python -m server.scripts.update_mds_cache` once per day (see `render.yaml`). The script downloads the latest JWS from the FIDO Alliance, verifies the signature against the trusted root, merges bundled PQC metadata, and stores a verified snapshot on the shared data volume (`/data/mds` by default).
...
- Render’s immutable image model means this cache must be generated at runtime; the cron job ensures the metadata stays fresh without slowing down app startup or consuming request-time resources.
