...
- `-q` quiet output, `-s` show print statements, `-k <expr>` filter tests by name.
- Examples:
  - `pytest -q -k webauthn`
  - `pytest -q tests/test_example.py::test_specific_case -s`
...
# Quickstart: create a virtual environment, run the demo server, and use the authenticator
...
- A supported web browser (Chrome/Edge ≥ 108, Safari ≥ 17, Firefox ≥ 117)
- An authenticator: platform (Windows Hello, Touch ID) or a security key
