...
The repository now contains a multi-stage `Dockerfile` that builds liboqs from source, installs the
`oqs` Python bindings, and runs the Flask demo with Gunicorn.  This container is self-contained and
works on Render or any other Docker-capable platform.
