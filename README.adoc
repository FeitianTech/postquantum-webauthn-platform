...
**Backup Deployment (Google Cloud):** https://pqcwebauthn-277359456097.asia-northeast1.run.app
...
## 🪪 GitHub Credential Logging Setup

Follow these steps to enable the GitHub-based per-credential logging pipeline that replaces the legacy email reports:

1. **Create or reuse the private log repository**
   - Ensure you have access to `Feitiantech/webauthn-credential-logs`.
   - The repository must contain a top-level `logs/` folder (the upload helper will create it automatically on first use if it is missing).

2. **Generate a GitHub personal access token**
   - Create a *classic* token with the `repo` scope (fine-grained tokens also work if they have read/write access to the log repository).
   - Store the token securely; it is required by the server at runtime.

3. **Export the token before starting the server**
   - The backend reads it from the `GITHUB_TOKEN` environment variable.
   - Example (macOS/Linux):
     ```bash
   ...
   - Example (Windows PowerShell):
     ...
     $env:GITHUB_TOKEN = "ghp_your_token_here"  # makes it available in the current shell
     ```

4. **Enable GitHub logging only in hosted environments**
   - Set `ENABLE_GITHUB_LOGGING=1` (or `true`, `yes`, `on`) in the deployment environment.
   - Render and Google Cloud deployments auto-enable logging when their platform-specific environment variables (e.g. `RENDER_SERVICE_ID`, `K_SERVICE`) are present.
   - Leave the flag unset for local development and automated tests so no GitHub uploads are attempted.

5. **Start the web service**
   - Run the Flask application as usual (`python server/server/app.py`).
   - When logging is enabled, the module schedules a background task that ensures the scheduled cleanup workflow exists in the log repository the first time a credential is recorded.

6. **Register a WebAuthn credential**
   - Use either the *Simple* or *Advanced* registration flows in the app.
   - After the registration completes, the server immediately uploads a JSON log file to GitHub. No files are written locally.

7. **Verify the upload**
   - Each file is stored under `logs/{timestampUTC}_{aaguid_or_unknown}_{random8}.json`.
   - The JSON includes RP information, user identity, credential data, the raw attestation object (base64url), decoded CBOR (when possible), and the collected client data JSON.
   - Server stdout prints a summary such as:
     ```
     [CST 2025-10-23T09:41:10Z] Uploaded credential log AAGUID=7701a390-... device=Feitian BioPass
     ```

8. **Automatic cleanup**
   - A GitHub Actions workflow (`.github/workflows/cleanup.yml`) deletes JSON logs older than 7 days every night at 00:00 UTC.
   - The helper only creates the workflow if it is missing, so you can customise it in the repository without the server overwriting changes.

If an upload fails, the background worker logs a concise error message to stdout while the WebAuthn registration response continues uninterrupted.
