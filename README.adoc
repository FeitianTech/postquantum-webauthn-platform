...
image:https://img.shields.io/codecov/c/github/FeitianTech/postquantum-webauthn-platform?branch=main&label=Test%20Coverage&logo=codecov[Test Coverage,link="https://app.codecov.io/gh/FeitianTech/postquantum-webauthn-platform"]
image:https://img.shields.io/github/license/FeitianTech/postquantum-webauthn-platform?label=License&logo=open-source-initiative[License,link="COPYING"]
...
**Main Deployment:** https://webauthnlab.tech[webauthnlab.tech]

**Backup Deployment:** https://pqcwebauthn.onrender.com[pqcwebauthn.onrender.com]
...
**Scope**
- Includes: Python, virtual environment, Flask, python-fido2, and Post-Quantum Crypto (PQC) algorithms.
- Currently supporting ML-DSA 44/65/87 for PQC

> **Note for Developers:**  
> Setting up and building all local dependencies (Python environment, Flask/FIDO2/Cryptography, PQC libraries, liboqs builds, HTTPS certificates, etc.) requires significant configuration time and toolchain setup.
> For most use cases, it is far more efficient to use the deployed platform directly instead of running on local environment.
...
- **Python 3.12+ ** with pip (https://www.python.org/downloads/)
...
The first launch writes a Flask session secret to `instance/session-secret.key`. Keep that file alongside the application so
