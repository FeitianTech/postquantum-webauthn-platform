...
### Option A: HTTPS mode (Recommended for full WebAuthn compatibility)

For full WebAuthn support including advanced features, run in HTTPS mode:

```bash
python examples/server/server/server.py
```

**Quick setup with convenience script:**
```bash
./setup_https.sh
```

**Manual setup for HTTPS:**
1. Install mkcert: https://github.com/FiloSottile/mkcert
2. Install the local CA: `mkcert -install`
3. Generate certificates: `mkcert localhost 127.0.0.1 ::1`
4. Copy certificates to server directory:
   ```bash
   cp localhost+1.pem localhost+1-key.pem examples/server/server/
   ```

You should see Flask start with:
```
Starting server in HTTPS mode at https://127.0.0.1:5000
```

Open https://127.0.0.1:5000 in your browser.

### Option B: HTTP mode (Limited WebAuthn support)

For quick testing with limited WebAuthn features:

```bash
python examples/server/server/server.py --http
```

You should see Flask start with:
```
Starting server in HTTP mode at http://localhost:5000
```

Open http://localhost:5000 in your browser.

**Note:** Some WebAuthn features may not work in HTTP mode due to browser security restrictions.
...
This error occurs when there's a mismatch between the WebAuthn Relying Party ID and the server's hostname:

- **For HTTPS mode:** Use `https://127.0.0.1:5000` (not `localhost`)
- **For HTTP mode:** Use `http://localhost:5000` with the `--http` flag

### SSL Certificate Issues

If you see SSL certificate errors:

1. Ensure you're using the exact hostname `127.0.0.1` in the browser
2. Use the provided `setup_https.sh` script for automatic certificate generation
3. Make sure mkcert CA is installed: `mkcert -install`

### Browser Compatibility

- Modern browsers support WebAuthn (Chrome, Firefox, Safari, Edge)
- Some features require HTTPS for security reasons
- Platform authenticators (Windows Hello, Touch ID) may have additional requirements 
