...
- The cron job preserves the previous snapshot if a refresh fails. The web UI simply reads the cached JSON/JWS; if no snapshot exists yet it triggers a one-time refresh (with a brief retry back-off) so first-time visitors do not wait for the next scheduled run.
