...
## Local HTTPS Setup for Flask (FIDO2/WebAuthn Testing)

Creating and use `cert.pem` and `key.pem` for local HTTPS development, enabling WebAuthn/FIDO2 features in your Flask app.
- Some WebAuthn features and settings requires your site to be served over HTTPS and won't work on HTTP (except for `localhost`).

---

### Step 1: Install OpenSSL

- **Windows:** Download and install [Win32 OpenSSL](https://slproweb.com/products/Win32OpenSSL.html).
- **macOS:** Open Terminal. OpenSSL is usually pre-installed.
- **Linux:** OpenSSL is usually pre-installed. If not, install it with:
  ```sh
...
Open your terminal (PowerShell, CMD, bash, etc.) with administrator access, and cd to your project directory: 
...
### Step 3: Generate Certificate and Key

Run the following command to create both files:
...
This will create the twon files below in your root directory:
