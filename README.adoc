...
1. Choose the **Docker** environment (Render labels this as “Runtime: Docker”) and point it at this
   repository. If you already created the service with the Python runtime, delete and recreate it so
   the Docker runtime is used—seeing logs such as “Using Python version …” means Render is still on
