...
def test_a_damaged_container_key_is_shown_by_its_summary_and_offset_as_an_invalid_key():
    ...
    assert (key.diagnostic, key.kind) == ("invalid(array[1] at offset 1)", "invalid")
    assert decode_payload_text("a181bf01ff05", lenient=True)["data"]["decodedValue"] == {
        "invalid(array[1] at offset 1)": 5
    }
