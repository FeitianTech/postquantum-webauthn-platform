...
one today (125 handlers; 29 lines carry two). And the decoder's scripts touch
innerHTML only to clear a container -- everything they show, much of it quoted
from the input, goes in through textContent.
"""
...
_CLEAR = re.compile(r"\.innerHTML\s*=\s*(''|\"\")\s*;")
...
def test_decoder_scripts_use_inner_html_only_to_clear():
    uses = [
    ...
    assert uses, "the decoder clears its containers with innerHTML = ''"
    assert [use for use in uses if not _CLEAR.search(use[1])] == []
