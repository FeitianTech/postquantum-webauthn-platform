...
def test_maybe_store_uploaded_metadata_file_skips_upload_when_identical_sha_exists(monkeypatch, uploads):
    ...
    monkeypatch.setattr(uploads, "git_blob_sha", lambda _content: blob_sha)
    monkeypatch.setattr(
        uploads,
        "github_list_directory",
        lambda _folder: [
...
def test_maybe_store_uploaded_metadata_file_adds_new_file_with_sanitized_name(monkeypatch, uploads):
    ...
    monkeypatch.setattr(uploads, "is_logging_enabled", lambda: True)
    monkeypatch.setattr(uploads, "git_blob_sha", lambda _content: "fresh-sha")
    monkeypatch.setattr(uploads, "github_list_directory", lambda _folder: [])
    monkeypatch.setattr(
        uploads,
        "github_upload_file",
        lambda *args, **kwargs: upload_calls.append((args, kwargs)),
