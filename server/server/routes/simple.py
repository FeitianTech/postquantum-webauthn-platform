...
def authenticate_begin():
    uname = request.args.get("email")
    stored_credentials = readkey(uname)
    if not stored_credentials:
        abort(404)

    credential_data_map: Dict[str, AttestedCredentialData] = {}
    for cred in stored_credentials:
        try:
            attested_data = extract_credential_data(cred)
        except Exception:
            continue

        if not isinstance(attested_data, AttestedCredentialData):
            continue

        try:
            credential_id_bytes = bytes(attested_data.credential_id)
        except Exception:
            continue

        credential_id_b64 = base64.urlsafe_b64encode(credential_id_bytes).decode("ascii").rstrip("=")
        credential_data_map[credential_id_b64] = attested_data

    if not credential_data_map:
        abort(404)
    ...
    credential_data_list: List[AttestedCredentialData] = list(credential_data_map.values())

    if raw_credentials:
        parsed_credentials, _ = _parse_client_credentials(raw_credentials)
        filtered_credentials: List[AttestedCredentialData] = []

        for candidate in parsed_credentials:
            try:
                candidate_bytes = bytes(candidate.credential_id)
            except Exception:
                continue

            candidate_b64 = base64.urlsafe_b64encode(candidate_bytes).decode("ascii").rstrip("=")
            matched = credential_data_map.get(candidate_b64)
            if matched is None:
                continue

            filtered_credentials.append(matched)

        if not filtered_credentials:
            abort(404)

        credential_data_list = filtered_credentials

    allowed_ids_for_session: List[str] = []
    for credential in credential_data_list:
        try:
            credential_bytes = bytes(credential.credential_id)
        except Exception:
            continue

        allowed_ids_for_session.append(
            base64.urlsafe_b64encode(credential_bytes).decode("ascii").rstrip("=")
        )

    session['simple_credentials_email'] = uname
    session['simple_credentials_allowed_ids'] = allowed_ids_for_session
...
def authenticate_complete():
    uname = request.args.get("email")
    expected_email = session.pop('simple_credentials_email', None)
    if expected_email is not None and expected_email != uname:
        abort(400)
    ...
    allowed_ids = session.pop('simple_credentials_allowed_ids', None)
    credential_data_list: List[AttestedCredentialData] = []

    if isinstance(allowed_ids, list) and allowed_ids:
        for credential_id in allowed_ids:
            matched = credential_data_map.get(credential_id)
            if matched is not None:
                credential_data_list.append(matched)

        if not credential_data_list:
            abort(400)
    else:
        credential_data_list = list(credential_data_map.values())
