...
def github_get_json(path: str) -> Optional[Tuple[Dict[str, Any], str]]:
    """Return the JSON payload and blob SHA for ``path`` if it exists."""

    url = _api_url(f"contents/{path}")
    try:
        _, raw = _request("GET", url)
    except urllib_error.HTTPError as exc:
        if exc.code == 404:
            return None
        raise
    ...
    if payload.get("encoding") != "base64":
        raise RuntimeError("Unexpected encoding for GitHub content response")

    decoded = base64.b64decode(content.encode("utf-8"))
    data = json.loads(decoded.decode("utf-8"))
    sha = payload.get("sha")
    if not isinstance(sha, str):
        raise RuntimeError("Missing SHA for GitHub content response")
    return data, sha
...
def github_upload_json(
    ...
    *,
    ...
    serialised = json.dumps(obj, ensure_ascii=False, indent=2, sort_keys=True) + "\n"
